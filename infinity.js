...
!(function (window, Math, $) {
    ...
    function updateStartIndex(listView, prepended) {
        ...
        for (index = startIndex, length = lastIndex; index < length; index += 1) {
    ...
    function repartition(listView) {
        ...
        for (index = 0, length = pages.length; index < length; index += 1) {
            currPage = pages[index];
            currItems = currPage.items;

            for (itemIndex = 0, pageLength = currItems.length; itemIndex < pageLength; itemIndex += 1) {
    ...
    ListView.prototype.find = function (findObj) {
        ...
        findObj.each(function () {
            ...
            if (page) {
                ...
                for (index = 0, length = pageItems.length; index < length; index += 1) {
    ...
    function indexWithinRange(listView, top, bottom) {
        ...
        if (prevDiff < 0) {
            // Search above
            for (index = startIndex - 1; index >= 0; index -= 1) {
                curr = pages[index];
                midpoint = curr.top + curr.height / 2;
            ...
            // Search below
            for (index = startIndex + 1, length = pages.length; index < length; index += 1) {
                curr = pages[index];
                midpoint = curr.top + curr.height / 2;
                diff = rangeMidpoint - midpoint;
                if (diff < 0) {
    ...
    DOMEvent = (function () {
        ...
        function scrollAll() {
            var index, length;
            for (index = 0, length = boundViews.length; index < length; index += 1) {
    ...
    Page.prototype.lazyload = function (callback) {
        ...
        if (!this.lazyloaded) {
            for (index = 0, length = $el.length; index < length; index += 1) {
